...
import {
  ACTIVITY_DT_SUGGESTION_LINK_FOLLOWED, BOOKMARK_TYPE_CLAUSE, SIDEBAR_TAB_SUGGESTION_RESULT, WORKSPACE_ID_DEFAULT,
} from '../../consts';
...
class SuggestionDetail extends Component {
	...
	async componentDidMount() {
	  const { selectedSuggestion, getCommentsFor } = this.props;
	  const copy = [...selectedSuggestion];
	  copy.reverse();

	  if (copy[0]) getCommentsFor(copy[0].clause_id);
	}
  ...
  renderDocumentInfo = () => {
    ...
    const document_types = _.uniq(_.flatMap(documents.map((d) => d.doc_title)).filter((x) => x));
    ...
    return (
      <div>
        ...
        <br />
        <br />
        {selectedSuggestion[0].party_name && <><span className="MetadataTitle">Parties:</span>  {this.getArrayAsTextSentence(selectedSuggestion[0].party_name, ' ')}</>}
        {selectedSuggestion[0].party_name && <><br /><br /></>}
        {selectedSuggestion[0].client_name && <><span className="MetadataTitle">Client:</span>  {this.getArrayAsTextSentence(selectedSuggestion[0].client_name, ' ')}</>}
        {selectedSuggestion[0].client_name && <><br /><br /></>}
          ...
          <LayoutRow>
            <CommandBar
              ...
              items={[{
							  ...
							  onClick: () => {
							    setSidebarTab(SIDEBAR_TAB_SUGGESTION_RESULT);
							  },
              }, {
							  key: 'sugs',
